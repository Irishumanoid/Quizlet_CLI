...
class Questions:
    ...
    def add_FRQ_questions(self, file, delimiter=" "):
...
def are_similar_words(word_one, word_two, thresh):
    map_one = {}
    map_two = {}
    for c in word_one:
...
def grade_FRQ(response, answer, word_thresh = 0.6, index_thresh = 0.5, tot_thresh = 0.5) -> bool:
    ...
    for i in range(min(len(resp_words), len(answer_words))):
        r = r_tuple[i]
        a = a_tuple[i]
        if are_similar_words(r[1], a[1], word_thresh):
            num_words_same += 1
            if r[0] == a[0]:
                num_indices_same += 1
